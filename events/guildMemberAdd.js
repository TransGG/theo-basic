...
module.exports = async (client, member) => {
    ...
    function buildGreetMessageContent(applicant) {
        return `${roleMention(client.config.roles.greeter)}, ${pickGreeterMessage(userMention(applicant.id))}`;
    }
    ...
    function buildGreetEmbeds(applicant) {
        return [
            new EmbedBuilder()
                .setColor(0xF5F5F5)
                .setTitle(`Welcome to Transonance, a place for trans people around music. ${pickEmoji()}`)
                .setDescription(
                    `${bold(`Welcome ${userMention(applicant.id)}! We're glad to finally meet you!`)}\n${italic('Why don\'t you check out some of the channels below to get started?')}\n\n<id:customize> - Assign some Roles!\n${channelMention(client.config.channels.introduce)} - Introduce Yourself!\n${channelMention(client.config.channels.general)} - Start Chatting!`,
                )
