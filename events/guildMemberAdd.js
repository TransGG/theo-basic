...
module.exports = async (client, member) => {
  ...
  const who = [
    ...
    "who listen to music",
    "who enjoy music",
    "that always have a song playing",
    "that always have a song stuck in their head",
    "who fall asleep with music",
    "who can't live without music",
    "who like to hear music every day",
    "who like to hear music every night",
    "who have a leaderboard of favorite songs",
    "who skip through hundreds of songs to find the best one",
    "who have a playlist for every mood",
    "who have a playlist for every occasion",
    "whose music always puts a smile on their face",
    "who always have a song for every situation",
    "whose music makes them euphoric",
    "who like to sing along to their favorite songs",
    "who like to share their taste in music",
    "who love to discover new music",
    "who constantly have background music",
    "who care about the quality of their music",
    "who have a favorite genre",
  ];
  ...
  function pickRandomElement(array) {
    if (picked[array] === undefined) {
      picked[array] = [];
    }

    if (picked[array].length === array.length) {
      // Reset the array
      picked[array] = [];
    }

    let element = array[Math.floor(Math.random() * array.length)];
    while (picked[array].includes(element)) {
      element = array[Math.floor(Math.random() * array.length)];
    }

    picked[array].push(element);
  ...
  function buildGreetEmbeds(applicant) {
    return [
      new EmbedBuilder()
        .setColor(0xf5f5f5)
        .setTitle(
          `Welcome to Transonance, a place for trans people ${pickWho()} ${pickEmoji()}`
        )
        .setDescription(
          `${bold(
            `Welcome ${userMention(
  ...
  const messageContent = buildGreetMessageContent(member);
  const embeds = buildGreetEmbeds(member);

  channel.send({ content: messageContent, embeds: embeds });
};
